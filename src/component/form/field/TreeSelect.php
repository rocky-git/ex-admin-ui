 ...
 * @method $this allowClear(bool $clear = false) 显示清除按钮														    boolean
 ...
 * 																宽度时会被忽略。false 时会关闭虚拟滚动                      boolean | number
 * @method $this dropdownStyle(mixed $style) 下拉菜单的样式														   	 	object
 * @method $this labelInValue(bool $in = false) 是否把每个选项的 label 包装到 value 中，会把 value
 ...
 * @method $this multiple(bool $clear = false) 支持多选（当设置 treeCheckable 时自动变为 true）							boolean
 ...
 * @method $this treeIcon(bool $icon = false) 是否展示 TreeNode title 前的图标，没有默认样式，如设置为 true，
 * 											需要自行定义图标相关样式                                                       boolean
 * @method $this showSearch(bool $show = false) 在下拉中显示搜索框(仅在单选模式下生效）										boolean
 * @method $this size(string $size = 'default') 选择框大小，可选 large small												string
 * @method $this treeCheckable(bool $check = false) 显示 checkbox														boolean
 * @method $this treeCheckStrictly(bool $strict = false) checkable 状态下节点选择完全受控（父子节点选中状态不再关联）
 * 											，会使得 labelInValue 强制为 true                                             boolean
 * @method $this treeData(mixed $data = []) treeNodes 数据，如果设置则不需要手动构造 TreeNode 节点（value 在整个树范围内唯一）	array<{value, label, children, [disabled, disableCheckbox, selectable]}>													    boolean
 ...
 * 										替换 treeNode 中 title,value,key,children 字段为 treeData 中对应的字段             object
 * @method $this treeDataSimpleMode(mixed $mode = false) 使用简单格式的 treeData，具体设置参考可设置的类型
 * (此时 treeData 应变为这样的数据结构: [{id:1, pId:0, value:'1', label:"test1",...},...], pId 是父节点的 id)               false|Array<{ id: string, pId: string, rootPId: null }>
 ...
 * @method $this treeExpandedKeys(bool $clear = false) 设置展开的树节点													string[] | number[]
 * @method $this treeLine(mixed $line = false) 是否展示线条样式，请参考 Tree - showLine									boolean | object
